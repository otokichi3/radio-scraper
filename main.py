...
FM802_URL = os.environ['FM802_URL']
BQ = bigquery.Client()
FMCOCOLO_URL = os.environ['FMCOCOLO_URL']
FMCOCOLO_TABLE = os.environ['FMCOCOLO_TABLE']

def fm802_onair_info(request):
    html = requests.get(FM802_URL)
...
def fmcocoro_onair_info(request):
    html = requests.get(FMCOCOLO_URL)
    ...
    table_id = PROJECT_ID + '.' + BQ_DATASET + '.' + FMCOCOLO_TABLE
    table = BQ.get_table(table_id)
    errors = BQ.insert_rows(table, insert_list)
    if errors != []:
        raise BigQueryError(errors)
